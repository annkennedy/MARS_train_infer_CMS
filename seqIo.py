...
FRAME_FORMAT_PNG_GRAY = 001 #PNG
FRAME_FORMAT_PNG_COLOR =002 #PNG
...
def tsSync(video_path, srTop,srFront):
    ...
    ##correlate timestamps form one view to another
    mapTs = {}
    if seqV ==1:
...
class seqIo_reader():
    ...
    def readHeader(self):
        ...
        elif self.imageFormat in(001,002):  self.ext = 'png'
...
class seqIo_writer():
    def __init__(self,filename,old_header):
        ...
        elif codec in ['monopng', 'imageFormat001']:        self.frmt = 001;self.nch = 1;self.ext = 'png'
        elif codec in ['png', 'imageFormat002']:            self.frmt = 002;self.nch = 3;self.ext = 'png'
...
class seqIo_crop():
    ...
    def __init__(self,fname,tname,frames):
...
class seqIo_toImgs():
    ...
    def __init__(self,fName,tDir=[],skip=1,f0=0,f1=np.inf,ext=''):
        sr = seqIo_reader(fName)
...
class seqIo_frImgs():
    ...
    def __init__(self,fName,header=[],aviName=[],Is=[],sDir=[],name='I',ndig=5,f0=0,f1=1e6):
...
class seqIo_convert():
    ...
    def __init__(self,fName,tName,imgFun,info=[],skip=1,f0=0,f1=np.inf):
        assert(fName!=tName)
        sr = seqIo_reader(fName)
...
class seqIo_toVid():
    ...
    def __init__(self, fName,ext='avi'):
