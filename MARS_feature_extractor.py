...
def generate_feature_list():
    ...
    for cam in ['top', 'front']:
        for mouse in ['m1', 'm2']:
            ...
            # feats[cam][mouse]['pixel_change_wholeframe'] = ['pixel_change', 'imgDf1', 'imgDf2', 'imgDf3','imgDf6',
            #                                                 'imgMean', 'imgStd']
    ...
    for cam, parts in zip(['top', 'front'], [parts_top, parts_front]):
        for mouse in ['m1', 'm2']:
            ...
            # feats[cam][mouse]['parts_pixel_change'] = [(p + '_pc') for p in parts]
...
def generate_lambdas():
    ...
    lam = {'ell_ang': {}, 'ell': {}, 'ell_area': {}, 'xy_ang': {}, 'xy': {}, 'xybd': {}, 'dt': {}, '2mdt': {},
           'd2t': {}, 'pix': {}, 'pix_local': {}, 'xyxy_ang': {}, 'xyxy': {}, 'bb': {}}
    ...
    # these are currently untested because I don't like them and don't think we should be using them.
    lam['pix']['imgMean'] = lambda frame2, frame1: np.mean(frame2)
    lam['pix']['imgStd'] = lambda frame2, frame1: np.std(frame2)
    lam['pix']['imgDf1'] = lambda frame2, frame1: np.mean(np.abs(frame2 - frame1)**2) / np.mean(frame1)**2
    lam['pix']['imgDf2'] = lambda frame2, frame1: (np.mean(np.abs(frame2 - frame1)) / np.mean(frame1))**2
    lam['pix']['imgDf3'] = lambda frame2, frame1: (np.mean(np.abs(frame2 - frame1) / (frame1 + eps)))**2
    lam['pix']['imgDf6'] = lambda frame2, frame1: np.mean(np.abs(frame2 - frame1)**2) / np.std(frame1)**2
    lam['pix']['pixel_change'] = lambda frame2, frame1: \
        (np.sum((frame2 - frame1) ** 2)) / float((np.sum((frame1) ** 2)))

    for p, part in enumerate(parts_front):
        lam['pix_local'][part + '_pc'] = lambda frame2, frame1, x1, y1, x0, y0, l, part=p: \
            pixel_change_local(lam, frame2, frame2, x1[part], y1[part], x0[part], y0[part], l)
...
def open_video(video_path):
    ...
    if ext == 'seq':
        ...
        num_frames = reader.header['numFrames']
        im_w = reader.header['width']
        im_h = reader.header['height']
        ...
        im_h = int(reader.get(cv2.CAP_PROP_FRAME_HEIGHT))
        im_w = int(reader.get(cv2.CAP_PROP_FRAME_WIDTH))
        fps = reader.get(cv2.CAP_PROP_FPS)
        if np.isnan(fps): fps = 30.

    return reader, num_frames, im_h, im_w, fps, ext


def read_frame(reader, f, ext):
    if ext == 'seq':
        frame = reader.getFrame(f - 1)[0]
    else:
        reader.set(cv2.CAP_PROP_POS_FRAMES, f - 1)
        _, frame = reader.read()
    frame = frame.astype(np.float32)
    return frame


def close_video(reader, ext):
    if not ext:
        return
    elif ext == 'seq':
        reader.close()
...
def extract_features(pose_fullpath, video_fullpath=[], use_grps=[], use_cam='top', use_mice=['m1','m2'],
    ...
    if video_fullpath:
        ...
        vr, num_frames, im_h, im_w, fps, ext = open_video(video_fullpath)
    else:
        video_name = ''
        vr = []
        ext = []
        ...
        if (('pixel_change_wholeframe' in use_grps) or ('parts_pixel_change' in use_grps)):
            raise Exception('Video path must be provided if you want to use pixel-based features')
    ...
    try:
        ...
        # estimate the size of the resident (neck-to-tail), and use this to scale all features.
        # there are other ways to scale features- could do units of cm, but we'd need user to provide
        # number of pixels per cm for their arena. (For Anderson lab videos, it's pix_per_cm = 37.795)
        dscale = np.median(mouse_length)
        ...
        for f in range(num_frames):
            ...
            for m, (xa, ya, xb, yb, xa0, ya0, xa00, ya00, boxa, boxb) in enumerate([m1_vals, m2_vals]):
                ...
                # single-mouse features. Lambda returns pixels, convert to mouselengths.
                ...
                # ellipse-based features. Lambda returns pixels, convert to mouselengths.
                ...
                # ellipse-based area features. Lambda returns pixels^2, convert to mouselengths^2.
                for feat in lam['ell_area'].keys():
                    featname = "_".join((use_cam, mouse_list[m], feat))
                    if featname in features:
                        track['data'][m, f, features.index(featname)] = lam['ell_area'][feat](ell) / (dscale ** 2)

                # velocity features. Lambda returns pix/frame, convert to mouselengths/second.
                for feat in lam['dt'].keys():
                    featname = "_".join((use_cam, mouse_list[m], feat))
                    if featname in features:
                        track['data'][m, f, features.index(featname)] = lam['dt'][feat](xa, ya, xa0, ya0) * fps / dscale

                # acceleration features. Lambda returns pix/frame^2, convert to mouselengths/second^2.
                ...
                # two-mouse features. Lambda returns pixels, convert to mouselengths.
                ...
                # two-mouse velocity features. Lambda returns pix/frame, convert to mouselengths/second.
                ...
                # environment-based features. Lambda returns pixels, convert to mouselengths.
                ...
                # pixel-based features. #################################################
                if not (('pixel_change_wholeframe' in use_grps) or ('parts_pixel_change' in use_grps)):
                    continue
                if f==0:
                    frame1 = read_frame(vr, f, ext)
                else:
                    frame1 = frame2
                frame2 = read_frame(vr, f, ext)

                # pixel-change features. Lambda returns pix/frame, convert to pix/second.
                for feat in lam['pix'].keys():
                    featname = "_".join((use_cam, mouse_list[m], feat))
                    if featname in features:
                        track['data'][m, f, features.index(featname)] = lam['pix'][feat](frame2, frame1) * fps

                # local pixel-change features. Lambda returns pix/frame, convert to pix/second.
                for feat in lam['pix_local'].keys():
                    featname = "_".join((use_cam, mouse_list[m], feat))
                    if featname in features:
                        track['data'][m, f, features.index(featname)] = \
                            lam['pix_local'][feat](frame2, frame1, xa, ya, xa0, ya0, dscale) * fps
